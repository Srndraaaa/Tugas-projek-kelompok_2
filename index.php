...
session_start();
include "koneksi.php";
$query = mysqli_query($conn, "SELECT * FROM jadwal_interview");
...
<tr>
    ...
    <td>
        <span class="status 
        <?php
            if($row['status']=='BELUM HADIR') echo 'status-belum';
            elseif($row['status']=='SELESAI') echo 'status-selesai';
            elseif($row['status']=='DIJADWALKAN') echo 'status-dijadwalkan';
            else echo 'status-dibatalkan';
        ?>">
        <?= $row['status']; ?>
        </span>
    </td>
